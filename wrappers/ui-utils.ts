...
export const promptString = async (
    ...
    const promptFinal = defaultValue ? `${prompt} (default: ${defaultValue}):` : prompt;
    do {
        const input = await ui.input(promptFinal);
        const finalInput = input || defaultValue || '';
        if (validationFn && !validationFn(finalInput)) {
            ui.write(errorMessage || 'Invalid input. Please try again.');
            continue;
        }
        return finalInput;
    } while (true);
...
export const promptJettonMetadata = async (ui: UIProvider): Promise<JettonMetaParams> => {
    ...
    const description = await promptString(
    ...
    const symbol = await promptString(
        'Enter the token symbol:',
        ui,
        (input) => /^[A-Z]{1,5}$/.test(input),
        'Symbol must be 1 to 5 uppercase letters.',
    );

    // const amount = await promptAmount('Enter jetton amount', ui);
